...
- MCP stdio tool listing exposed 35 tools.
...
- No dedicated IOCTL buffer decoder is implemented yet. Use `windbg_driver_dispatch_snapshot`, `windbg_read_memory`, and raw `dt`/`dq` commands for buffer analysis until a driver-specific decoder is added.
